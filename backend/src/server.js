...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error.message === 'play_developer_api_not_configured') return sendJson(res, 503, { error: 'verification_unavailable' });
    sendJson(res, 500, { error: 'internal_error' });
  }
});

server.listen(PORT, HOST, () => {
  safeLog(`Quietype backend listening on http://${HOST}:${PORT}`);
});
